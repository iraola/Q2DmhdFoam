...
from IDM_meshAndGo import meshAndGo
...
def plot_plot(values1, values2, x, labels, **kwargs):
    ...
    plt.show()
"""
def validation_tepot(Ha_list, Re_list, Gr_list):
    '''
    '''
    ...
    for Re in Re_list:
        for Gr in Gr_list:
            for Ha in Ha_list:
                tag_dict['b'] = tag_dict['a'] * 1
                maxW[i], maxW_th[i] = meshAndGo(Ha, Re, Gr,
                                 tag_dict=tag_dict, phys_dict=phys_dict, IDM_dict=IDM_dict)
                print('\n' + str(maxW[i]) + '\n' )
                i = i + 1
                # Save directory for analysis
                sp.call("mv case case_" + "Re" + str(Re) + "Gr" + str(Gr) +"Ha" + str(Ha), shell=True)
...
Gr_list=[2e8]
Re_list=[5e3]
validation_tepot(Ha_list, Re_list, Gr_list)
