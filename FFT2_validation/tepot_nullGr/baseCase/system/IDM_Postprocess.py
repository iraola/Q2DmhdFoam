...
U_mean = 0.00020533333333333336**2        #Re * nu / (L/2) (optional)
...
def HV(field_vector,L,n,slope,interception,r2):             #e.g HV(a1[1],L,n)
    ...
    FFT_file= open("IDM_output_Vort.txt","w+")
...
a1=readProbes(n, path, fileName, 180)  #Imported DATA
hh=np.array([])
...
HV(a1[1],Ly,n,np.average(hh0),np.average(hh1),np.average(hh2))          #Execution & storage KEC/HV results
...
if plot_chart == True:
    ...
    Fx =[np.fft.rfftfreq(2**n,Ly/2**n)[1],np.fft.rfftfreq(2**n,Ly/2**n)[-1]]
    Fy =[1e5,0.0005]
    ax.loglog(np.fft.rfftfreq(2**n,Ly)*2**n, np.square(np.abs(np.fft.rfft(a1[3][2**(n-2)])/(2**(n))/U_mean)),'.-',label='FFT')
    ax.loglog(Fx, Fy, '--', label='Regresion')
    ax.set_title('FFT')
    #ax.set_ylim([1e-2,1e2])
    ax.grid()
    plt.legend(loc='best')
    tex = r'$\varepsilon(k) \backsim k^{'+'% 1.1f' % np.average(hh0)+'}$'
    ax.text(200, 3, tex, fontsize=12, va='bottom')
    ax.set(xlabel='Wavenumber [1/m]', ylabel='Amplitude')
    plt.subplots_adjust(left=0.14, bottom=0.2, right=0.9, top=0.9, wspace=0.0, hspace=0.0)    
    plt.savefig('Poiseuille_FFT',dpi=600)
    #APLITUDE LOG SPECTRUM
    fig, (ax0, ax1, ax2) = plt.subplots(nrows=3, figsize=(9, 9))
    ax0.loglog(np.fft.rfftfreq(2**n,Ly)*2**n, np.sqrt(np.abs(np.fft.rfft(a1[3][2**(n-2)])/(2**(n)))/U_mean),'.-')
    ax0.set_title('APLITUDE LOG SPECTRUM')
    ax0.set_ylim([1e-4,1e4])
    ax0.grid()
    ax0.set(xlabel='Wavenumber [1/m]', ylabel='Amplitude [Adimentionalized field units]')
    #POWER LOG SPECTRUM
    ax1.loglog(np.fft.rfftfreq(2**n,Ly)*2**n, np.abs(np.fft.rfft(a1[3][2**(n-2)])/(2**(n))/U_mean),'.-')
    ax1.set_title('POWER LOG SPECTRUM')
    ax1.set_ylim([1e-4,1e6])
    ax1.grid()
    ax1.set(xlabel='Wavenumber [1/m]', ylabel='Power [Adimentionalized squared field units]')
    #FIELD PROFILE
    ...
    #plt.show()
    plt.savefig('Poiseuille_FFT_Vort',dpi=600)
...
if plot_chart_2D == True:
    ...
    ax.set_title('FIELD PROFILE [Adimentionalized field units]')
    cbar = fig.colorbar(cs)
    plt.savefig('Poiseuille_FieldProfile_Vort',dpi=600)
    #2D profile spectrum
    fig, ax = plt.subplots(figsize=(6, 3))
    ...
    cs=ax.imshow(kl_1[0:2**(n-5),0:2**(n-4)], interpolation="none", cmap='Spectral',
        origin='lower',extent=[0, freq[2**(n-4)], 0, freq[2**(n-5)]],vmax=np.max(kl_1), vmin=0)
